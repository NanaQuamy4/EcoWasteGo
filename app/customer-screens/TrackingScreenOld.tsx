...
const mockWasteCollection = {
  ...
  recycler_id: "00000000-0000-0000-0000-000000000002",
...
export default function TrackingScreen() {
  ...
  const fetchNotifications = useCallback(async () => {
    if (!trackingData?.customerName) return;
  ...
  }, [params.requestId, trackingData?.customerName]);
    ...
    try {
      const { error } = await supabase
        .rpc('mark_notification_read', {
          ...
          p_customer_id: trackingData?.customerName // This should be customer ID, but using name for now
        });
    ...
    try {
      ...
      if (requestError) {
        ...
        Alert.alert(
          ...
          `Failed to load tracking information: ${requestError.message}`,
          [{ text: 'OK' }]
        );
        return;
      }
      ...
      if (requestData) {
        ...
        // Set waste collection with real data (this was missing!)
        setWasteCollection({
          id: requestData.id,
          customer_id: requestData.customer_id,
          recycler_id: requestData.recycler_id,
          waste_type: requestData.waste_type || 'Mixed Waste',
          weight: requestData.waste_quantity || 1,
        ...
        console.log('⚠️ No tracking data found, using mock data');
        setTrackingData({
          requestId: params.requestId,
          customerName: 'Customer',
          customerPhone: 'Unknown',
          recyclerName: 'Recycler',
          recyclerPhone: 'Unknown',
  ...
  const updateRecyclerLocation = useCallback(async () => {
    ...
    // Skip database query if using mock data or invalid UUID format
    const uuidRegex = /^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i;
    const isMockData = wasteCollection.recycler_id === "00000000-0000-0000-0000-000000000002" || 
                      wasteCollection.recycler_id === "user_002" ||
                      !uuidRegex.test(wasteCollection.recycler_id);
    
    if (isMockData) {
      console.log('Using mock data or invalid UUID - skipping database query:', wasteCollection.recycler_id);
      return;
    }
    ...
    try {
      ...
      if (recyclerData?.latitude && recyclerData?.longitude) {
        ...
        // Check if recycler has started navigation (status = 'in_progress')
        const isNavigating = trackingData?.status === 'in_progress';
        ...
        if (customerLocation) {
          ...
          // Only track movement if recycler has started navigation
          if (isNavigating && previousLocation) {
            ...
            // Recycler hasn't started navigation yet
            setIsTruckMoving(false);
            setMovementSpeed(0);
            console.log('🚛 Recycler is stationary - waiting to start navigation');
          }
        }
      }
    } catch (error) {
  ...
  }, [wasteCollection?.recycler_id, customerLocation, recyclerLocation, trackingData?.status]);
  ...
  // REMOVED: Mock arrival timer - now using real database arrival detection only
  // The arrival status is now properly detected from the database via checkArrivalStatus()
            ...
            <View style={styles.mapHeader}>
              ...
              <Text style={styles.mapSubtitle}>
                {hasReachedDestination
                  ...
                  ? `${trackingData?.recyclerName || 'Recycler'} is preparing to start navigation • ${distanceToCustomer.toFixed(1)} km away`
              ...
              <View style={styles.movementIndicator}>
                <View style={styles.movementIcon}>
                  <MaterialIcons 
                    name={
                      trackingData?.status === 'in_progress' 
                        ? (isTruckMoving ? "local-shipping" : "pause-circle-filled")
                        : "schedule"
                    } 
                    size={24} 
                    color={
                      trackingData?.status === 'in_progress'
                        ? (isTruckMoving ? COLORS.green : COLORS.orange)
                        : COLORS.blue
                    } 
                  />
                </View>
                <View style={styles.movementInfo}>
                  <Text style={styles.movementStatus}>
                    {trackingData?.status === 'in_progress'
                      ...
                      : '⏳ Recycler is preparing to start navigation'
                    }
                  </Text>
                  {trackingData?.status === 'in_progress' && isTruckMoving && movementSpeed > 0 && (
                  ...
                  {trackingData?.status === 'confirmed' && (
                    <Text style={styles.movementSpeed}>
                      Waiting for recycler to start navigation...
                    </Text>
                  )}
                </View>
              </View>
                  ...
                  markers={[
                    {
                      ...
                      type: 'recycler',
                      isMoving: trackingData?.status === 'in_progress' && isTruckMoving && !hasReachedDestination,
                    },
                    {
                      id: 'destination',
