...
-- 2. Create verification history table for audit trail
...
DROP TRIGGER IF EXISTS check_verification_expiration_trigger ON recyclers;
...
-- 12. Update existing recyclers to have pending status
UPDATE recyclers 
...
-- 16. Insert default admin role (you'll need to update this with your actual admin user ID)
-- Uncomment and update the line below after creating your admin account
...
-- Show new columns in recyclers table
SELECT column_name, data_type, is_nullable 
FROM information_schema.columns 
WHERE table_name = 'recyclers' 
AND column_name IN ('admin_verified', 'verification_status', 'verification_expires_at', 'admin_notes')
ORDER BY column_name;

-- Show new tables
SELECT table_name 
FROM information_schema.tables 
WHERE table_name IN ('verification_history', 'admin_roles')
ORDER BY table_name;
