-- =====================================================
-- FINAL FIX FOR ADMIN FUNCTIONS - MATCH ACTUAL DATABASE TYPES
-- =====================================================

-- The error shows that the actual columns are TEXT type, not VARCHAR
-- Let's fix the function to match the actual database schema
...
-- Using TEXT types to match the actual database schema
CREATE OR REPLACE FUNCTION admin_get_all_recyclers_status()
RETURNS TABLE (
  id UUID,
  full_name TEXT,           -- Changed back to TEXT to match actual schema
  phone TEXT,              -- Changed back to TEXT to match actual schema
  email TEXT,              -- Changed back to TEXT to match actual schema
  truck_size TEXT,         -- Changed back to TEXT to match actual schema
  rating NUMERIC,
  verification_status TEXT, -- Changed back to TEXT to match actual schema
  is_available BOOLEAN,
  is_online BOOLEAN,
  last_seen_at TIMESTAMP WITH TIME ZONE,
  heartbeat_at TIMESTAMP WITH TIME ZONE,
  session_id TEXT,         -- Changed back to TEXT to match actual schema
  created_at TIMESTAMP WITH TIME ZONE,
  status_category TEXT     -- Changed back to TEXT to match actual schema
) AS $$
BEGIN
  RETURN QUERY
  SELECT 
