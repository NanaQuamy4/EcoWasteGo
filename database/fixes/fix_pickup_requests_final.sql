-- Final fix for pickup_requests table with error handling
-- This handles all edge cases and existing configurations
...
-- Step 4: Create missing customer records (preserves existing data)
...
-- Step 6: Add missing columns to pickup_requests table
DO $$ 
BEGIN
...
-- Step 8: Create indexes (only if they don't exist)
...
-- Step 13: Enable real-time for pickup_requests table (with error handling)
DO $$ 
BEGIN
    ...
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added pickup_requests to real-time publication';
    ELSE
        RAISE NOTICE 'pickup_requests already in real-time publication';
    END IF;
EXCEPTION
    WHEN OTHERS THEN
        RAISE NOTICE 'Real-time setup skipped (already configured or not available)';
END $$;

-- Step 14: Insert sample data if table is empty
