-- Fix RLS Policies for Recyclers Table - Version 3
-- This script completely fixes the infinite recursion issue by recreating policies
...
-- Step 2: Drop ALL existing policies (without IF EXISTS since we know they exist)
DROP POLICY "Simple recycler access" ON recyclers;
DROP POLICY "Simple customer access" ON customers;
DROP POLICY "Simple verification access" ON recycler_verifications;
DROP POLICY "Simple admin access" ON admin_users;
