-- Recreate pickup_requests table from scratch
-- Use this if the table structure is completely broken

-- Drop the table if it exists (WARNING: This will delete all data!)
DROP TABLE IF EXISTS pickup_requests CASCADE;

-- Create the pickup_requests table with proper structure
...
CREATE INDEX idx_pickup_requests_pickup_date ON pickup_requests(preferred_pickup_date);
CREATE INDEX idx_pickup_requests_coordinates ON pickup_requests(pickup_latitude, pickup_longitude);
